...
class TrainingKD:
    ...
    def train_one_epoch(self, freq=1, steps_per_epoch=None, tq=None):
        ...
        for i, data in enumerate(self.train_loader):
            ...
            if self.regularizers_optics_ce is not None:
                ...
                final_loss += tmp
                txt_reg_ce = "".join([f"{key}: {reg_ce[key]:.2f}, " for key in reg_ce.keys()])
            ...
            if self.regularizers_optics_mo is not None:
                ...
                final_loss += tmp
                txt_reg_mo = "".join([f"{key}: {reg_mo[key]:.2f}, " for key in reg_mo.keys()])
            ...
            txt_losses = "".join([f"{key}: {loss_values[key]:.2f}, " for key in loss_values.keys()])
    ...
    def fit(self, n_epochs, verbose_reg=True, freq=1, steps_per_epoch=None):
        ...
        for epoch in range(n_epochs):
            ...
            with tqdm(total=len(self.train_loader), dynamic_ncols=True, colour="blue") as tq:
