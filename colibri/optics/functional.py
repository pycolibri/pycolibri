...
def forward_dd_cassi(x: torch.Tensor, ca: torch.Tensor) -> torch.Tensor:
    ...
    assert (
        ca.shape[-1] == N + L - 1
    ), "The coded aperture must have the same size as a dispersed scene"
...
def backward_spc(
    y: torch.Tensor, H: torch.Tensor, pinv: bool = False, binary: bool = False
) -> torch.Tensor:
    r"""
...
def get_spatial_coords(
    M: int, N: int, pixel_size: float, device=torch.device("cpu"), type="cartesian"
) -> tuple:
    r"""
...
def transfer_function_angular_spectrum(
    M: int,
    N: int,
    pixel_size: float,
    wavelengths: torch.Tensor,
    ...
    fr = fr.unsqueeze(0)
    H = torch.exp(
        1j * wave_number(wavelengths) * distance * (1 - ((fr**2) * wavelengths**2)) ** 0.5
    )

    return H


def fraunhofer_propagation(
    field: torch.Tensor,
    M: int,
    N: int,
    pixel_size: float,
    ...
    r = r.unsqueeze(0)
    c = (
        torch.exp(1j * wave_number(wavelengths) * distance)
        / (1j * wavelengths * distance)
        * torch.exp(1j * wave_number(wavelengths) / (2 * distance) * (r**2))
    )
...
def fraunhofer_inverse_propagation(
    field: torch.Tensor,
    pixel_size: float,
    wavelengths: torch.Tensor,
    distance: float,
...
def scalar_diffraction_propagation(
    field: torch.Tensor, distance: float, pixel_size: float, wavelength: list, approximation: str
) -> torch.Tensor:
    r"""
    ...
    if approximation == "fresnel":
        ...
        H = transfer_function_fresnel(M, N, pixel_size, wavelength, distance, field.device)
    elif approximation == "angular_spectrum":
        H = transfer_function_angular_spectrum(M, N, pixel_size, wavelength, distance, field.device)

    elif approximation == "fraunhofer":
        return fraunhofer_propagation(field, M, N, pixel_size, wavelength, distance, field.device)

    elif approximation == "fraunhofer_inverse":
        return fraunhofer_inverse_propagation(field, pixel_size, wavelength, distance, field.device)
    else:
...
def height2phase(
    height_map: torch.Tensor, wavelengths: torch.Tensor, refractive_index: callable
) -> torch.Tensor:
    r"""
...
def psf_single_doe_spectral(
    ...
    k0 = wave_number(wavelengths)
    doe = height2phase(
        height_map=torch.unsqueeze(height_map, 0),
    ...
    optical_field_in_sensor = scalar_diffraction_propagation(
        field=optical_field,
...
def add_pad(x: torch.Tensor, pad: list) -> torch.Tensor:
    ...
    assert len(x.shape) == len(
        pad
    ), "The tensor and the padding must have the same number of dimensions"
...
def unpad(x: torch.Tensor, pad: list) -> torch.Tensor:
    ...
    if len(x.shape) == 4:
        return x[
            (0 + pad[0]) : (x.shape[0] - pad[0]),
            (0 + pad[1]) : (x.shape[1] - pad[1]),
            (0 + pad[2]) : (x.shape[2] - pad[2]),
            (0 + pad[3]) : (x.shape[3] - pad[3]),
        ]
    elif len(x.shape) == 3:
        return x[
            (0 + pad[0]) : (x.shape[0] - pad[0]),
...
def signal_conv(image: torch.Tensor, psf: torch.Tensor) -> torch.Tensor:
    ...
    image = torch.nn.functional.conv2d(
        image, torch.flip(psf, [-2, -1]), padding=(N - 1, N - 1), groups=C
    )

    new_size = image.shape[-2:]
    image = unpad(
        image,
        pad=[0, 0, (new_size[0] - original_size[0]) // 2, (new_size[1] - original_size[1]) // 2],
    )
    return image
